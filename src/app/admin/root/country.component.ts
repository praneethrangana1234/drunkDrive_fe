...
import { CountryaddComponent } from 'src/app/countryadd/countryadd.component';
@Component({
...
constructor(private _dialog: MatDialog, private _coreService: CoreService, private http:HttpClient ,private sanitizer: DomSanitizer,private countryService: CountryService)
{
...
openaddemp()
{
 const dialogRef= this._dialog.open(CountryaddComponent);
...
getEmployeeList()
{
  this.countryService.getEmployeeList().subscribe({
next: (res) =>{
console.log(res);
...
{
  console.log(id);
this.countryService.deleteemployee(id).subscribe({
next: (res) =>{
  console.log("okkk");
