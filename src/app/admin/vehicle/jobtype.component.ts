...
import { JobtypeaddComponent } from 'src/app/jobtypeadd/jobtypeadd.component';
...
export class JobtypeComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'jobType', 'code','action'];
  dataSource!: MatTableDataSource<any>;

  @ViewChild(MatPaginator) paginator!: MatPaginator;
  @ViewChild(MatSort) sort!: MatSort;
constructor(private _dialog: MatDialog, private _coreService: CoreService, private http:HttpClient ,private sanitizer: DomSanitizer,private jobtypeService: JobtypeService)
{
...
openaddemp()
{
 const dialogRef= this._dialog.open(JobtypeaddComponent);
...
getEmployeeList()
{
  this.jobtypeService.getVehicleList().subscribe({
next: (res) =>{
console.log(res);
...
{
  console.log(id);
this.jobtypeService.deleteVehicle(id).subscribe({
next: (res) =>{
  console.log("okkk");
